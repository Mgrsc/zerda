...
- `custom_primitives/run_zerda_with_deps.sh` scans `custom_primitives/**/requirements.txt`, installs merged dependencies into the unified PTC Python runtime referenced by `ZERDA_PTC_PYTHON` (default `/opt/zerda-python/bin/python`), where the bundled image creates that runtime via `uv venv --python 3.13`, installs Playwright Chromium when `playwright` is declared, caches dependency state under `~/.zerda/custom_primitives/`, and only then starts `zerda`
...
- `custom_primitives/scrapling/requirements.txt` declares both `scrapling[fetchers]` and `playwright`, so `run_zerda_with_deps.sh` will install the Python packages and the Chromium browser runtime automatically into the unified PTC Python runtime for Scrapling stealth fetches.
