...
- Primitive discovery starts from prompt-visible top-level primitive names plus runtime `help(...)`.
...
5. `src/ptc/parser.rs` parses exact `<PTC_TOOL_CALLING>` blocks.
6. `src/ptc/primitive_index.rs` scans internal, external, and custom primitives for metadata.
7. `src/ptc/job_manager.rs` launches Python jobs, tracks status, and reinjects runtime results.
