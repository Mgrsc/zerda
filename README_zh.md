...
Zerda 在 Executor 路径引入 ACON（Agent Context Optimization），把记忆优化目标从“持续喂任务知识”转为“沉淀可复用的方法论与教训”（`How to act / What to avoid`）。每次执行前，系统会对委托指令做向量化检索，从 Qdrant 召回最相似的历史指南，并以精简的 `<system-reminder>` 注入到 Executor 提示词中。
...
蒸馏后的指南会写回向量库，作为后续相似指令的先验。系统同时包含负反馈回收机制：若注入指南后任务最终仍失败，会删除本轮注入的指南条目，避免无效经验在后续任务中被持续放大。
